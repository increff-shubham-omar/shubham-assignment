...
export const DeliveredTrips = ({ data }: { [key: string]: TripDataProps[] | any }) => {
  ...
  return (
    <Card className="basis-1/3 border-none px-3 py-2 shadow-xl shadow-gray-200 dark:shadow-gray-800/70 dark:bg-gray-700" shadow="none">
      <CardBody className="flex flex-row gap-6 justify-between leading-9">
        <div className="grow flex flex-col justify-between">
        <h6 className="text-lg">Delivered</h6>
        <h1 className="text-4xl font-semibold">{Intl.NumberFormat('en-IN').format(data?.['DELTrips']?.length)}</h1>
        </div>
        ...
        <CircularProgress
          ...
          {Intl.NumberFormat('en-IN').format(data?.['onTimeTrips']?.length)}
          </span>
        </p>}
